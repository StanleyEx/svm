...
public:
  ...
  void diagEmit(DiagnosticLevel level, const SourceLocation location,
    ...
    std::vsnprintf(buffer, size + suffixSize + 1, fmt, args);
...
#define SVM_NOTE(ENGINE, SRCLOC, FMT, ...)                                     \
  do {                                                                         \
    (ENGINE).diagEmit(::svm::DiagnosticLevel::Note, (SRCLOC), __FILE__,        \
                      __func__, __LINE__, (FMT)__VA_OPT__(, ) __VA_ARGS__);    \
  } while (0)
